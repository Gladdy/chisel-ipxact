...
val output_data = UInt(OUTPUT, 64)
val output_ready = UInt(OUTPUT, 64)
val output_test = UInt(OUTPUT, 64)
}
...
class AXI4LiteSlave extends Module {
  val io = new Bundle {
    ...
    // attached peripheral
    // XXX replace
    val badhash_instance = new badhash_interface
val badhash_2_instance = new badhash_2_interface
...
val badhash_2_output_test_readbuffer = Reg(init = UInt(0, 64))
...
}.elsewhen(rvalid && addr === UInt(64)) {
  io.axi.rchannel.valid := Bool(true)
  io.axi.rchannel.bits.rdata := badhash_2_output_test_readbuffer
}
...
badhash_2_output_test_readbuffer := io.badhash_2_instance.output_test
...
val badhash_2_input_ready_writebuffer = Reg(init = UInt(0, 64))
  ...
  */
  ...
  // XXX replace
  when(wvalid && addr === UInt(4160782352) && io.axi.wchannel.valid) {
  badhash_output_data_writebuffer := io.axi.wchannel.bits.wdata
}.elsewhen(wvalid && addr === UInt(4160782360) && io.axi.wchannel.valid) {
  badhash_output_ready_writebuffer := io.axi.wchannel.bits.wdata
}.elsewhen(wvalid && addr === UInt(48) && io.axi.wchannel.valid) {
  badhash_2_output_data_writebuffer := io.axi.wchannel.bits.wdata
}.elsewhen(wvalid && addr === UInt(56) && io.axi.wchannel.valid) {
  badhash_2_output_ready_writebuffer := io.axi.wchannel.bits.wdata
}.elsewhen(wvalid && addr === UInt(64) && io.axi.wchannel.valid) {
  badhash_2_output_test_writebuffer := io.axi.wchannel.bits.wdata
}
